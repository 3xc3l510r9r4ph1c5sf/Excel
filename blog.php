...
if ( function_exists( 'algenix_elementor_is_preview' ) && algenix_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			algenix_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	algenix_new_main_query(
		array(
			'post_type'      => algenix_get_theme_option( 'post_type' ),
			'category'       => algenix_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'algenix_filter_get_template_part', algenix_blog_archive_get_template() ) );
}
